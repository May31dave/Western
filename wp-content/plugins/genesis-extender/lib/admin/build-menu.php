...
function genesis_extender_build_admin_submenus()
{
	...
	if( !get_the_author_meta( 'disable_genesis_extender_settings_menu', $user->ID ) && CHILD_THEME_NAME != 'Dynamik Website Builder' )
	{
	...
	if( !get_the_author_meta( 'disable_genesis_extender_custom_menu', $user->ID ) && CHILD_THEME_NAME != 'Dynamik Website Builder' )
	{
...
function genesis_extender_admin_init()
{
	...
	wp_register_script( 'genesis_extender_jquery_tooltip', GENEXT_URL . 'lib/js/jquery.tools.min.js' );
...
function genesis_extender_admin_styles()
{
	...
	wp_enqueue_script( 'genesis_extender_jquery_tooltip' );
}
...
/**
 * Execute the genesis_extender_write_files() function when the Genesis "Theme Settings"
 * admin page is loaded. This ensures the Genesis Extender Stylesheets are re-written based
 * on the latest "Default Layout" and "Header Settings" settings.
 *
 * @since 1.0
 */
add_action( 'admin_head', 'genesis_extender_settings_write_styles' );
